...
from appointment.utils.db_helpers import (
    Appointment, AppointmentRequest, EmailVerificationCode, Service, Worker, WorkingHours, calculate_slots,
...
def fetch_user_appointments(user):
    ...
    try:
        staff_member_instance = user.Worker
...
def prepare_user_profile_data(user, staff_user_id):
    ...
    bt_help = Worker._meta.get_field('appointment_buffer_time')
    bt_help_text = bt_help.help_text

    sd_help = Worker._meta.get_field('slot_duration')
