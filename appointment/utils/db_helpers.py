...
Worker = apps.get_model('appointment', 'Worker')
...
def get_non_working_days_for_staff(staff_member_id):
    ...
    try:
        staff_member = Worker.objects.get(id=staff_member_id)
...
def get_staff_member_appointment_list(staff_member: Worker) -> list:
...
def get_staff_member_buffer_time(staff_member: Worker, date: datetime.date) -> float:
...
def get_staff_member_end_time(staff_member: Worker, date: datetime.date) -> Optional[datetime.time]:
...
def get_staff_member_from_user_id_or_logged_in(user, user_id=None):
    """Fetch Worker based on the user_id or the logged-in user."""
    staff_member = None
    try:
        if user_id:
            staff_member = Worker.objects.get(user_id=user_id)
        else:
            staff_member = user.Worker
    except Worker.DoesNotExist:
        pass
    return staff_member


def get_staff_member_slot_duration(staff_member: Worker, date: datetime.date) -> int:
...
def get_staff_member_start_time(staff_member: Worker, date: datetime.date) -> Optional[datetime.time]:
...
def get_working_hours_for_staff_and_day(staff_member, day_of_week):
    ...
    if not working_hours and not (start_time and end_time):
        return None
    # If no specific working hours are set for that day, use the default start and end times from Worker
...
def is_working_day(staff_member: Worker, day: int) -> bool:
